...
# NOTE: This no longer works as XML file structure has changed.
namespace :dinesafe do
  ...
  task :geocode => :environment do
    Geocode.where("geocoding_results_json IS NULL").first(2500).each do |g|
      ...
      map_url = "https://maps.googleapis.com/maps/api/geocode/json?address=#{address},+toronto,+on&key=#{Dinesafe.conf.google_maps_geocoding_api_key}"
